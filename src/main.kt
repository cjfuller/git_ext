    ...
    companion object {
        fun parse(str: String?): Status? {
            ...
            return if (match != null) {
                Status(
                    ahead = match.groupValues[1].let { if (it == "") null else it.toInt() },
                    behind = match.groupValues[2].let { if (it == "") null else it.toInt() },
                )
            } else {
                null
            }
...
fun formatTreeRootedAt(
    ...
    when {
        ...
        listOf(
            ...
            root.desc.sha,
            root.desc.status?.ahead?.let { green("+$it") } ?: "",
            root.desc.status?.behind?.let { brightRed("-$it") } ?: "",
